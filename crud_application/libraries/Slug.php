...
class Slug extends CI_Model
{
	 ...
	 * @return string
	 */
	public function get_keybyslug($data = '')
	{
		if (is_string($data))
		{
			$CI =& get_instance();

			$CI->db->select($this->id)->where($this->field, $data);
	...
	public function get_slugbykey($data = '')
	{
		if (is_integer($data))
		{
			$CI =& get_instance();

			$CI->db->select($this->field)->where($this->id, $data);
			$query = $CI->db->get($this->table);			
